...
STATE="MD"
LOCALITY="Madrid"
ORGANIZATION_NAME="iot-tst"
root_ca_password="1234"
key_bits_length="4096"
days_till_expire=30
ca_chain_prefix="iottst.chain.ca"
intermediate_ca_dir="."
...
root_ca_prefix="iottst.root.ca"
intermediate_ca_prefix="iottst.intermediate"

openssl_device_config_file="./openssl_device_intermediate_ca.cnf"
...
function generate_device_certificate_common()
{
    ...
    openssl req -new -sha256 \
        ...
        -subj "$(makeCNsubject "${common_name}")" \
		-out ${certificate_dir}/csr/${device_prefix}.csr.pem
    [ $? -eq 0 ] || exit $?
...
# Generates a certificate for verification, chained directly to the root.
###############################################################################
function generate_verification_certificate_sinterm()
{
    if [$# -ne 1]; then
        echo "Usage: <subjectName>"
        exit 1
...
if [ "${1}" == "create_root_and_intermediate" ]; then
...
elif [ "${1}" == "create_verification_certificate_sinterm" ]; then
    generate_verification_certificate_sinterm "${2}"	
    ...
    echo "       create_verification_certificate <subjectName>  # Creates a verification certificate, signed by root CA with <subjectName>"
    echo "       create_verification_certificate_sinterm <subjectName>  # Creates a verification certificate, signed by intermediate CA with <subjectName>"	
